...
Run `c2w-admin connection list` for the actual buckets and their state.
...
```bash
c2w-admin settings set transfer.bandwidth_limit_mbps 200
c2w-admin settings set source.concurrency_per_connection 5   # CommPeak's own recommendation
c2w-admin settings set transfer.concurrency_global 20
```
...
```bash
c2w-admin doctor
journalctl -u 'c2w-worker@*' -f
```

Rising `pending` with flat `transferred` means workers are blocked, not slow —
check `error_class` distribution with the `c2w-diagnose` skill.
