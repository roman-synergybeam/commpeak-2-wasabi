---
name: cprec-migrate
description: Create or apply an Alembic migration for cprec, including the Row-Level Security policies and brand partitions that every tenant-scoped table needs. Use when adding or changing a table, adding a brand, or when RLS/partition boilerplate is required.
---
...
3. a policy keyed off the `cprec.brand_id` session variable,
...
CREATE POLICY my_table_brand_isolation ON my_table
  USING (brand_id = current_setting('cprec.brand_id', true)::bigint)
  WITH CHECK (brand_id = current_setting('cprec.brand_id', true)::bigint);
...
CREATE POLICY my_table_platform ON my_table TO cprec_platform USING (true);
```
...
`cprec-admin brand add` does this in the same transaction as the brand row.
