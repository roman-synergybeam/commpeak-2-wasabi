...
| `AUTH_ERROR` | token/secret wrong or rotated | Re-enter credentials via `cprec-connection`; do not retry first |
...
| `RATE_LIMIT` | throttled | `cprec-admin settings set source.concurrency_per_connection 5`; the ladder already backs off |
...
running — `systemctl status 'cprec-worker@*'`.
...
```bash
journalctl -u 'cprec-worker@*' -p warning --since '1 hour ago'
journalctl -u cprec-api -f
```
