...
Enter credentials only through `cprec-admin`, which prompts without echo and
...
```bash
cprec-admin destination add --brand go4rex --provider wasabi \
  --region eu-central-1 --bucket go4rex-recordings --path-prefix archive
cprec-admin destination list
```
...
1. Check `cprec-admin doctor` — it reports schema revision, brands, users,
...
3. Use the `cprec-backfill` skill before enabling transfers on a bucket with
   years of history.
