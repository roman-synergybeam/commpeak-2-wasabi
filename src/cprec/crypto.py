...
(``CPREC_MASTER_KEY``, delivered by systemd ``LoadCredential=``).
...
Ciphertext layout (then urlsafe-base64 encoded)::
    ...
    b"cprec1" | key_id (16 bytes) | nonce (12 bytes) | ciphertext+tag
"""
...
def _load_master_key() -> bytes:
    ...
    if not raw:
        raise CryptoError(
            "CPREC_MASTER_KEY (or CPREC_MASTER_KEY_FILE) is not set. Generate one with "
    ...
    try:
        ...
        raise CryptoError("CPREC_MASTER_KEY is not valid urlsafe base64") from exc
    if len(key) != _KEY_LEN:
        raise CryptoError(f"CPREC_MASTER_KEY must decode to {_KEY_LEN} bytes, got {len(key)}")
    return key
