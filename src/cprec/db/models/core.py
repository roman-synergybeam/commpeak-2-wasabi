...
the ``cprec.brand_id`` session variable.  Application ``WHERE`` clauses are a
...
class CommPeakConnection(Base, IdMixin, TimestampMixin):
    ...
    #: Sealed by cprec.crypto under the brand's data key. Never logged, never
