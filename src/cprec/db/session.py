...
brand.  ``brand_session`` does that by setting the ``cprec.brand_id`` session
...
from cprec.config import Bootstrap, get_bootstrap
...
def get_engine(bootstrap: Bootstrap | None = None) -> AsyncEngine:
    ...
    if _engine is None:
        ...
        _engine = _build_engine(str(cfg.database_url), "cprec")
    return _engine
...
def get_platform_engine(bootstrap: Bootstrap | None = None) -> AsyncEngine:
    ...
    if _platform_engine is None:
        cfg = bootstrap or get_bootstrap()
        _platform_engine = _build_engine(cfg.effective_platform_url, "cprec-platform")
...
async def brand_session(brand_id: int | None) -> AsyncIterator[AsyncSession]:
    ...
    async with factory() as session:
        await session.execute(
            text("SELECT set_config('cprec.brand_id', :brand, true)"),
