"""``cprec-admin`` -- operator command line.
...
from cprec import __version__
from cprec.auth.local import AuthError, create_super_admin, hash_password, revoke_all_sessions
from cprec.config import get_bootstrap
from cprec.crypto import CryptoError, generate_data_key, seal
from cprec.db.models.auth import User
from cprec.db.models.core import Brand, CommPeakConnection, StorageDestination, Tenant
from cprec.db.session import dispose_engine, platform_session
from cprec.logging import configure_logging
from cprec.settings import SettingsError, settings_service
from cprec.settings_spec import SETTINGS, specs_by_category
from cprec.storage.commpeak import COMMPEAK_ENDPOINT
...
async def cmd_user_list(_args: argparse.Namespace) -> int:
    async with platform_session() as session:
        ...
        if not rows:
            ...
            _out("  cprec-admin superadmin create --email you@example.com")
            return EXIT_OK
...
async def cmd_settings_set(args: argparse.Namespace) -> int:
    ...
    if spec is None:
        _out(f"Unknown setting: {args.key}")
        _out("List available settings with: cprec-admin settings list")
        return EXIT_ERROR
...
async def cmd_settings_reveal(args: argparse.Namespace) -> int:
    ...
    it can be used directly:
        ...
        export SHADCNIO_TOKEN=$(cprec-admin settings reveal integrations.shadcn_mcp_token)
    """
    spec = SETTINGS.get(args.key)
    if spec is None:
        _out(f"Unknown setting: {args.key}")
...
async def cmd_brand_list(_args: argparse.Namespace) -> int:
    async with platform_session() as session:
        ...
        if not rows:
            _out("No brands yet. Create one with: cprec-admin brand add --name X --slug x")
            return EXIT_OK
...
async def cmd_connection_add(args: argparse.Namespace) -> int:
    ...
    async with platform_session() as session:
        ...
        _out(f"Test it read-only with: cprec-admin connection test --id {conn.id}")
    return EXIT_OK
...
async def cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    try:
        async with platform_session() as session:
            ...
            if users == 0:
                _out("\n  Next: cprec-admin superadmin create --email you@example.com")
            elif brands == 0:
                _out("\n  Next: cprec-admin brand add --name 'Go4Rex' --slug go4rex")
            elif dests == 0:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cprec-admin",
        description="Administer the cprec recording platform. All configuration is stored "
        "in the database; there are no config files to edit.",
    )
    parser.add_argument("--version", action="version", version=f"cprec {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    configure_logging("cprec-admin", level_name="WARNING", json_output=False)
    ...
    try:
        ...
        _out("Check CPREC_DATABASE_URL in the systemd unit, and that PostgreSQL is running.")
        return EXIT_ERROR
