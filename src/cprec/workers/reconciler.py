...
from cprec.db.base import JobKind, RecordingState, SyncRunKind
...
from cprec.storage.factory import open_destination
from cprec.sync import queue
from cprec.sync.transfer import verify_recording
from cprec.workers.scheduler import singleton_lock
...
async def _amain() -> None:
    configure_logging("cprec-reconciler")
    async with platform_session() as session:
        await reconfigure_from_settings("cprec-reconciler", session)

    async with singleton_lock("cprec-reconciler") as acquired:
        if not acquired:
