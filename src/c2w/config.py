...
:mod:`c2w.settings_spec` and :mod:`c2w.settings`).  Exactly two values
cannot, because they are what a process needs in order to reach the database
and to make sense of what it finds there:

* ``C2W_DATABASE_URL`` -- where the settings are.
* ``C2W_MASTER_KEY`` -- the key that unseals stored credentials.  Without it
...
``C2W_PLATFORM_DATABASE_URL`` is optional and only differs from the main URL
...
class Bootstrap(BaseSettings):
    ...
    database_url: PostgresDsn = Field(
        default="postgresql+asyncpg://c2w@127.0.0.1:5432/c2w"  # type: ignore[arg-type]
    )
    ...
    def _read_key_file(self) -> Bootstrap:
        ...
        if self.master_key_file is not None and not self.master_key.get_secret_value():
            try:
                ...
                raise ValueError(
                    f"cannot read C2W_MASTER_KEY_FILE at {self.master_key_file}: {exc}"
                ) from exc
